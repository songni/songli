...
gulp.task('copy:view', () => {
    return gulp.src(`${serverPath}/views/**/*`)
        .pipe(gulp.dest(`${paths.dist}/server/views`))
})

gulp.task('copy:ico', () => {
    return gulp.src(`${clientPath}/favicon.ico`)
        .pipe(gulp.dest(`${paths.dist}/${clientPath}`))
})
            ...
            gulp.src(`${paths.dist}/${clientPath}/app/bundle.js`, {read: false}),
                {
                    ...
                    endtag: '<!-- endbuild -->',
                    transform: filepath => '<script src="' + filepath.replace(`/${paths.dist}\/${clientPath}/`, '') + '"></script>'
                }))
        .pipe(gulp.dest(`${paths.dist}/${clientPath}`));
})
            ...
            gulp.src(`${paths.dist}/${clientPath}/app/bundle.js`, {read: false}),
                {
                    starttag: '<!-- build:js(dist/client) app/bundle.js -->',
                    endtag: '<!-- endbuild -->',
                    transform: filepath => "<script>var body = document.getElementsByTagName('body')[0];var script = document.createElement('script');script.src = window.location.origin + ':8080' + '/client/app/bundle.js';body.appendChild(script);</script>"
                }))
...
gulp.task('copy', cb => {
    runSequence(['copy:html', 'copy:view', 'copy:conf', 'copy:ico'], cb);
})
